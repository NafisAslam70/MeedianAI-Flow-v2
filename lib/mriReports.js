...
const AHR_TEMPLATE_KEY = "academic_health_report";
const AHR_FAMILY_KEY = "amri";

const PHONE_CALL_TEMPLATE_KEY = "phone_call_drive";
const PHONE_CALL_FAMILY_KEY = "amri";

const HOSTEL_DAILY_DUE_TEMPLATE_KEY = "hostel_daily_due_report";
const HOSTEL_FAMILY_KEY = "amri";
...
const AHR_TEMPLATE_DEFINITION = {
  key: AHR_TEMPLATE_KEY,
  name: "Academic Health Report",
  description:
    "Evening dean daily audit covering Slot 12 supervision, MHCP-2 conductance, deanship escalations, and shutdown actions.",
  allowPreSubmit: false,
  defaultFrequency: "daily",
  instructions:
    "Assign this report to the evening dean. The dean completes the report from Close My Day → MRI Clearance → Academic Health. Managers review submissions from the Academic Health console.",
  formSchema: { sections: [] },
  meta: {
    version: 1,
    schema: "academic_health_v1",
  },
};

const PHONE_CALL_TEMPLATE_DEFINITION = {
  key: PHONE_CALL_TEMPLATE_KEY,
  name: "Phone Call Drive",
  description:
    "Campaign log for outbound guardian phone calls covering focus area, outcomes, and follow-up actions.",
  allowPreSubmit: true,
  defaultFrequency: "adhoc",
  instructions:
    "Use this template to coordinate guardian phone call drives. Update each contact attempt with status and notes, then summarise campaign outcomes before closing.",
  formSchema: {
    sections: [
      {
        key: "callSheet",
        ...
        fields: [
          ...
          { id: "phone", type: "text", label: "Phone number" },
          {
            ...
            type: "select",
            label: "Call status",
            options: ["Pending", "Connected", "No Answer", "Wrong Number", "Callback Scheduled"],
          },
          { id: "notes", type: "textarea", label: "Notes / commitments" },
        ],
      },
      {
        key: "campaignSummary",
        ...
        fields: [
          {
            ...
            options: ["Pending", "In Progress", "Completed", "Requires Escalation"],
          },
          { id: "nextSteps", type: "textarea", label: "Next steps / escalations" },
        ],
      },
    ],
  },
  meta: {
    version: 1,
    schema: "phone_call_drive_v1",
  },
};

const HOSTEL_DAILY_DUE_TEMPLATE_DEFINITION = {
  key: HOSTEL_DAILY_DUE_TEMPLATE_KEY,
  name: "Hostel Daily Due Report",
  description:
    "Daily report by hostel incharge tracking dues, student involvement, actions taken, and authorization signatures.",
  allowPreSubmit: true,
  defaultFrequency: "daily",
  instructions:
    "Fill this report daily to track hostel dues and related activities. Select students involved and record actions taken with proper authorization.",
  formSchema: {
    sections: [
      {
        key: "dueRows",
        title: "Daily Due Report",
        repeat: true,
        fields: [
          { id: "sn", type: "number", label: "SN" },
          { id: "particulars", type: "text", label: "Particulars" },
          { id: "studentInvolved", type: "select", label: "Any Student Involved", options: "students_by_class" },
          { 
            id: "actionType", 
            type: "select", 
            label: "Who Will Take Action", 
            options: ["Student Self", "Higher Authority"] 
          },
          { 
            id: "actionDetails", 
            type: "textarea", 
            label: "Action Details", 
            conditional: { field: "actionType", value: "Student Self" } 
          },
          { 
            id: "higherAuthorityAction", 
            type: "textarea", 
            label: "Higher Authority Action", 
            conditional: { field: "actionType", value: "Higher Authority" } 
          },
          { 
            id: "followUpStatus", 
            type: "select", 
            label: "Status", 
            options: ["Needs Follow-up", "Done"] 
          },
          { 
            id: "needsEscalation", 
            type: "select", 
          ...
          { id: "authSign", type: "text", label: "Auth Sign" },
        ],
      },
    ],
  },
...
const toISODate = (input) => {
  ...
  const istDate = formatISTDate(date);
  if (!istDate) throw new Error("Failed to format IST date");
  return istDate;
};
...
export async function ensureAcademicHealthTemplate() {
  return ensureReportTemplate(AHR_TEMPLATE_DEFINITION, AHR_FAMILY_KEY);
}

export async function ensurePhoneCallTemplate() {
  return ensureReportTemplate(PHONE_CALL_TEMPLATE_DEFINITION, PHONE_CALL_FAMILY_KEY);
}

export async function ensureHostelDailyDueTemplate() {
  return ensureReportTemplate(HOSTEL_DAILY_DUE_TEMPLATE_DEFINITION, HOSTEL_FAMILY_KEY);
}
...
const mapAhrStatusToMriStatus = (status) => {
  const normalized = String(status || "").toUpperCase();
  if (normalized === "APPROVED") return "verified";
  if (normalized === "SUBMITTED") return "submitted";
  if (normalized === "REOPENED") return "draft";
...
export async function ensureAcademicHealthAssignmentsForUser(userId, targetDate) {
  if (!userId) return [];
  ...
  const [template] = await db
    ...
    .from(mriReportTemplates)
    .where(and(eq(mriReportTemplates.key, AHR_TEMPLATE_KEY), eq(mriReportTemplates.active, true)))
    .limit(1);
  if (!template?.id) return [];
...
export async function getMemberReports({ userId, targetDate }) {
  ...
  const academicHealthAssignments = await ensureAcademicHealthAssignmentsForUser(userId, isoDate);

  const assignments = Array.from(
    new Map(
      [...ptAssignments, ...subjectAssignments, ...academicHealthAssignments].map((assignment) => [
        assignment.id,
        assignment,
      ])
    ).values()
  ...
  for (const report of reports) {
    if (report.templateKey === AHR_TEMPLATE_KEY) {
      const instance = instancesByAssignment.get(report.assignmentId);
      const mergedMeta = safeJson(report.meta);
      const siteId = Number(mergedMeta?.siteId) || 1;
      const checkMode = mergedMeta?.checkMode || "MSP";
      const academicReport = await createAcademicHealthReport({
        reportDate: isoDate,
        siteId,
        assignedToUserId: userId,
        checkMode,
      });
      const mappedStatus = mapAhrStatusToMriStatus(academicReport.status);
      let instanceMeta = safeJson(instance?.meta, {});
      let metaNeedsUpdate = instanceMeta.ahrReportId !== academicReport.id;
      if (metaNeedsUpdate) {
        instanceMeta = { ...instanceMeta, ahrReportId: academicReport.id };
      }
      if (instance && (instance.status !== mappedStatus || metaNeedsUpdate)) {
        await db
          .update(mriReportInstances)
          .set({
            status: mappedStatus,
            meta: metaNeedsUpdate ? instanceMeta : instance.meta,
            updatedAt: new Date(),
          })
          .where(eq(mriReportInstances.id, instance.id));
      ...
      processedReports.push({
        ...report,
        status: instance?.status ?? mappedStatus,
        academicHealth: academicReport,
        meta: {
          ...mergedMeta,
          ahrReportId: academicReport.id,
          siteId,
          checkMode: academicReport.checkMode,
        },
      });
      continue;
    }
