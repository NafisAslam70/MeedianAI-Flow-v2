...
const MOP_PROGRAM_KEY = "MOP";
const MOP2_TRACK = "mop2";
...
async function fetchLatestMop2Checkin({ userId, reportDate }) {
  ...
  const rows = await db
    ...
    .innerJoin(scannerSessions, eq(scannerSessions.id, attendanceEvents.sessionId))
    .where(
      and(
        eq(attendanceEvents.userId, Number(userId)),
        eq(scannerSessions.programKey, MOP_PROGRAM_KEY),
        eq(scannerSessions.track, MOP2_TRACK),
        gte(attendanceEvents.at, start),
        lt(attendanceEvents.at, end)
      )
    )
