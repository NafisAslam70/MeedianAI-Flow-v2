  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        gte(finalDailyAttendance.at, start),
        lt(finalDailyAttendance.at, end)
      )
    )
    .orderBy(desc(finalDailyAttendance.at))
    .limit(1);

  if (!rows?.length) {
    console.info("[AcademicHealth] No MHCP attendance rows", {
      userId,
      reportDate,
    ...
    if (diagnostics.length) {
      console.info("[AcademicHealth] Attendance diagnostics", diagnostics);
    } else {
      console.info("[AcademicHealth] No attendance rows at all for user/day", { userId, reportDate });
    }
    return null;
  }
  const row = rows[0];
...
const validateBeforeSubmit = (report) => {
  ...
  if (!report.mop2CheckinId || !report.mop2CheckinTime) {
    errors.push("MOP2 check-in not found. Please scan attendance before submitting.");
  }
