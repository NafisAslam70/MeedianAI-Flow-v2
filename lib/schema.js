...
export const assignedTaskLogs = pgTable("assigned_task_logs", {
  ...
  details: text("details"), // e.g., JSON or text describing changes
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const routineTaskLogs = pgTable("routine_task_logs", {
  ...
  routineTaskId: serial("routine_task_id").references(() => routineTasks.id).notNull(),
  userId: serial("user_id").references(() => users.id).notNull(),
  action: text("action").notNull(), // e.g., "created", "updated", "status_changed"
  details: text("details"), // e.g., JSON or text describing changes
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const generalLogs = pgTable("general_logs", {
  id: serial("id").primaryKey(),
  userId: serial("user_id").references(() => users.id).notNull(),
  action: text("action").notNull(), // e.g., "login", "logout", "profile_updated"
  details: text("details"), // e.g., JSON or text describing the action
  createdAt: timestamp("created_at").defaultNow().notNull(),
});