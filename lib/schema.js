...
export const MRI_ROLE_OPTIONS = [
  ...
  "sports_moderator",
  "util_moderator",
  "pt_moderator", // Parent Teacher (class teacher) moderator
];
...
export const mriProgramRoles = pgTable("mri_program_roles", {
  ...
  action: varchar("action", { length: 32 }).notNull(), // open | close | substitute | approve | check
  roleKey: varchar("role_key", { length: 64 }).notNull(), // e.g., MSP-MOD, MSP-Pre-MOD (freeform for now)
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (t) => ({
  uniqGrant: uniqueIndex("mri_program_roles_unique").on(t.programId, t.action, t.roleKey),
}));
...
// Tasks attached to MRI Role Definitions
