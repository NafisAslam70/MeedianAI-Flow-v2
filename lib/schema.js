...
export const statusEnum = pgEnum("status", [
  ...
  "verified",
  "done",
  "not_done",  // Added for potential future use
]);
...
export const users = pgTable("users", {
  ...
  whatsapp_number: varchar("whatsapp_number", { length: 15 }), // Nullable, as per change
...
export const dailySlotLogs = pgTable("daily_slot_logs", {
  ...
  idx_slot_date: index("idx_daily_slot_logs_slot_date").on(table.slotId, table.date), // Suggested: For common queries
}));
...
export const routineTaskLogs = pgTable("routine_task_logs", {
  id: serial("id").primaryKey(),
  routineTaskId: integer("routine_task_id").references(() => routineTasks.id), // Nullable
...
export const routineTaskDailyStatuses = pgTable("routine_task_daily_statuses", {
  ...
  idx_routine_task_date: index("idx_routine_task_daily_statuses_task_date").on(table.routineTaskId, table.date), // Suggested
}));
...
export const assignedTaskLogs = pgTable("assigned_task_logs", {
  ...
  idx_task_id: index("idx_assigned_task_logs_task_id").on(table.taskId), // Suggested
}));
...
export const messages = pgTable("messages", {
  ...
  idx_sender_recipient: index("idx_messages_sender_recipient").on(table.senderId, table.recipientId), // Suggested for chat queries
}));
...
export const announcements = pgTable("announcements", {
  ...
  subject: varchar("subject", { length: 255 }).notNull(), // Made required as per comment
  content: text("content").notNull(),
...
export const dayCloseRequests = pgTable("day_close_requests", {
  ...
  assignedTasksUpdates: jsonb("assigned_tasks_updates"), // Store task updates as JSON
  routineTasksUpdates: jsonb("routine_tasks_updates"), // Store routine task updates as JSON
  ...
  idx_user_date: index("idx_day_close_requests_user_date").on(table.userId, table.date), // Suggested
}));
...
export const notCompletedTasks = pgTable("not_completed_tasks", {
  ...
  userId: integer("user_id").references(() => users.id).notNull(),
  date: timestamp("date").notNull(),
  details: jsonb("details").notNull(), // JSON with task info (e.g., title, comment, etc.)
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
