...
export const meedSchedules = pgTable(
  "meed_schedules",
  {
    ...
    description: text("description"),
    scheduleNumber: varchar("schedule_number", { length: 40 }),
    releasedBy: varchar("released_by", { length: 160 }),
    ...
    goal: text("goal"),
    printOptions: jsonb("print_options").default(JSON.stringify({})),
    active: boolean("active").notNull().default(true),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
  },
  (t) => ({
    ...
    idxModerator: index("meed_schedules_moderator_idx").on(t.moderatorId),
  })
);
...
export const meedScheduleDivisions = pgTable(
  "meed_schedule_divisions",
  {
    ...
    dutyHolders: jsonb("duty_holders").default(JSON.stringify([])),
    duties: jsonb("duties").default(JSON.stringify({})),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (t) => ({
...
/* -------------------- MHCP Slot Duties -------------------- */
export const mhcpSlotDuties = pgTable(
  "mhcp_slot_duties",
  {
    id: serial("id").primaryKey(),
    programId: integer("program_id").notNull().references(() => mriPrograms.id, { onDelete: "cascade" }),
    track: varchar("track", { length: 32 }).notNull().default("both"),
    dayName: varchar("day_name", { length: 16 }).notNull(),
    slotLabel: varchar("slot_label", { length: 80 }).notNull(),
    dutyTitle: varchar("duty_title", { length: 160 }),
    startTime: time("start_time"),
    endTime: time("end_time"),
  ...
  (t) => ({
    idxProgramDay: index("mhcp_slot_duties_program_day_idx").on(t.programId, t.dayName),
    idxProgramTrack: index("mhcp_slot_duties_program_track_idx").on(t.programId, t.track),
  })
);

/* -------------------- MHCP Seeds -------------------- */
export const mhcpSeeds = pgTable(
  "mhcp_seeds",
  {
    id: serial("id").primaryKey(),
    programId: integer("program_id").notNull().references(() => mriPrograms.id, { onDelete: "cascade" }),
  ...
  (t) => ({
    idxProgramTrack: index("mhcp_seeds_program_track_idx").on(t.programId, t.track),
  })
);
