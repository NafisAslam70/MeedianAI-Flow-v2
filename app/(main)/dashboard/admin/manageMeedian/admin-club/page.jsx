...
export default function AdminClubPage() {
  ...
  const { summary: ptSummary, error: ptError, isLoading: ptLoading } = usePtSummary(isAdmin, date);
  const { summary: gateSummary, error: gateError, isLoading: gateLoading, warning: gateWarning } = useGateSummary(isAdmin, date);
  const { data: programsData, error: programsError, isLoading: programsLoading } = useSWR(
    isAdmin ? "/api/admin/manageMeedian?section=metaPrograms" : null,
    fetcher,
    { dedupingInterval: 5 * 60_000 }
  );
  const { data: teamData, error: teamError, isLoading: teamLoading } = useSWR(
    isAdmin ? "/api/admin/manageMeedian?section=team" : null,
    fetcher,
    { dedupingInterval: 5 * 60_000 }
  );
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
            ...
            <div className="grid gap-4 md:grid-cols-[2fr_1fr]">
              <div className="space-y-4">
                ...
                <div className="overflow-hidden rounded-xl border border-slate-100">
                  <table className="min-w-full text-sm">
                    ...
                    <tbody>
                      {Array.from(ptSummary.statuses.entries())
                        .sort((a, b) => a[0].localeCompare(b[0]))
                        .map(([status, count]) => (
                          <tr key={status} className="border-t border-slate-100">
                            <td className="px-3 py-2 capitalize text-slate-700">{status}</td>
                            <td className="px-3 py-2 text-right font-semibold text-slate-900">{count}</td>
                          </tr>
                        ))}
                    </tbody>
                  </table>
              ...
              <div className="flex flex-col gap-3">
                <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                  Verify submitted diaries promptly to keep the closes flowing. Pending counts include both drafts and missing submissions.
                </div>
                <Link
                ...
                <Link
                  ...
                  className="inline-flex items-center justify-center rounded-lg bg-gray-900 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-black/80"
                >
                  Manage PT Template
                </Link>
              </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Campus In / Out</h2>
          <StatusBadge label={`${gateSummary.totals?.out || 0} out`} tone={gateSummary.totals?.out ? "warn" : "default"} />
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          {gateLoading ? (
            <p className="text-sm text-slate-500">Loading gate activity…</p>
          ) : gateError ? (
            <p className="text-sm text-rose-600">Failed to load gate logs: {gateError.message}</p>
          ) : (
            <div className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-xl border border-slate-100 bg-slate-50 p-3 text-sm">
                  <p className="text-xs uppercase tracking-wide text-slate-500">Logs captured</p>
                  <p className="mt-1 text-2xl font-semibold text-slate-900">{gateSummary.count || 0}</p>
                </div>
                <div className="rounded-xl border border-emerald-100 bg-emerald-50/60 p-3 text-sm">
                  <p className="text-xs uppercase tracking-wide text-emerald-600">Returned</p>
                  <p className="mt-1 text-2xl font-semibold text-emerald-900">{gateSummary.totals?.in || 0}</p>
                </div>
                <div className="rounded-xl border border-amber-100 bg-amber-50/70 p-3 text-sm">
                  <p className="text-xs uppercase tracking-wide text-amber-600">Stepped out</p>
                  <p className="mt-1 text-2xl font-semibold text-amber-900">{gateSummary.totals?.out || 0}</p>
                </div>
              </div>
          {gateWarning && (
            <div className="rounded-lg border border-amber-200 bg-amber-50/80 px-3 py-2 text-xs text-amber-800">
              {gateWarning === "purposeColumnMissing"
                ? "Gate logs loaded without purpose notes. Run the latest migration to capture visit purposes."
                : gateWarning === "missingTable"
                ? "Gate logs table missing. Run the campus gate migration to enable this view."
                : `Gate logs warning: ${gateWarning}`}
            </div>
          )}
          {gateSummary.latest?.length ? (
                <div className="overflow-hidden rounded-xl border border-slate-100">
                  <table className="min-w-full text-sm">
                    <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
                      <tr>
                    ...
                    <tbody>
                      {gateSummary.latest.map((log) => (
                        <tr key={log.id} className="border-t border-slate-100">
                          <td className="px-3 py-2 text-slate-700">{log.userName || "—"}</td>
                          <td className="px-3 py-2 capitalize text-slate-700">{log.direction || "—"}</td>
                          <td className="px-3 py-2 text-slate-500">{log.purpose || "—"}</td>
                ...
                <p className="text-sm text-slate-500">No gate activity recorded for this date.</p>
              )}
              <div className="flex flex-wrap gap-2">
                ...
                <Link
                  href="/dashboard/member/gate"
                  className="inline-flex items-center justify-center rounded-lg border border-transparent px-4 py-2 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-100"
                >
                  Launch campus hub
                </Link>
              </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Daily MRI Journal</h2>
          {selectedMemberId && (
            <div className="flex items-center gap-2 text-xs text-slate-500">
              <StatusBadge label={`${journalSessions.length} entries`} tone={journalBadgeTone} />
              {journalSessions.length ? <span>Total {totalJournalDurationLabel}</span> : null}
            </div>
          )}
        </div>
        <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:gap-4">
            <div className="w-full md:w-64">
              <Select
                ...
                disabled={teamLoading || !memberOptions.length}
              >
                <option value="" disabled>
                  {memberOptions.length ? "Select member" : "Loading members…"}
                </option>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1" htmlFor="journal-date">
                Date
              </label>
              <input
                id="journal-date"
            ...
            <button
              ...
              disabled={!selectedMemberId || journalLoading}
          ...
          {teamLoading ? (
            ...
            <p className="text-sm text-rose-600">Failed to load team list: {teamError.message}</p>
          ) : !memberOptions.length ? (
            <p className="text-sm text-slate-500">No members available yet.</p>
          ) : !selectedMemberId ? (
            <p className="text-sm text-slate-500">Pick a member to view their MRI journal.</p>
          ) : (
            <div className="space-y-4">
              <div className="text-xs text-slate-500">
                Viewing MRI journal for{" "}
                <span className="font-semibold text-slate-700">{selectedMemberName}</span> on{" "}
                <span className="font-semibold text-slate-700">{journalDateLabel}</span>.
              </div>
              {journalLoading ? (
                <p className="text-sm text-slate-500">Loading journal entries…</p>
              ) : journalError ? (
                <p className="text-sm text-rose-600">
                  Failed to load MRI journal: {journalError.message}
                </p>
              ) : journalSessions.length === 0 ? (
                <p className="text-sm text-slate-500">
                  No MRI journal entries recorded for this member on the selected date.
                </p>
              ) : (
                <div className="space-y-3">
                  {journalSessions.map((session) => {
                    const durationMinutes = computeDurationMinutes(session.startedAt, session.endedAt);
                    return (
                      <div
                        key={session.id}
                        className="rounded-xl border border-slate-100 bg-slate-50/80 p-3 transition hover:border-teal-200"
                      >
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                          <div>
                            <p className="text-sm font-semibold text-slate-900">
                              {session.itemTitle || "Untitled activity"}
                            </p>
                        ...
                        {session.note && (
                          <p className="mt-2 rounded-lg border border-slate-200 bg-white/80 px-3 py-2 text-xs text-slate-700">
                            {session.note}
                          </p>
                        )}
                      </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Attendance — All Programs</h2>
          <div className="inline-flex items-center gap-2 text-xs text-slate-500">
            <Download size={14} />
            <span>Export from the attendance report view</span>
          </div>
        </div>
