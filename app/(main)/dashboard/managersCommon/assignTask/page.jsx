...
function ManageTasksModal({ tasks, members, onClose, onEditTask, onManageSprints }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
  ...
  const handleDeleteTask = async (taskId) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(
          errorData.error || `HTTP ${response.status}: ${response.statusText || "Failed to delete task"}`
        );
      }

      setPreviousTasks((prev) => prev.filter((task) => task.id !== taskId));
      if (newTask?.id === taskId) setNewTask(null);
      ...
      setError(
        `Error deleting task: ${err.message}. Please check if the DELETE endpoint is correctly configured at /api/managersCommon/assign-tasks/[taskId].`
      );
    } finally {
      setLoading(false);
      setLoadingAction("");
        ...
        <div className="w-full h-full bg-white rounded-2xl shadow-xl p-4 sm:p-6 flex flex-col sm:flex-row gap-4 sm:gap-6">
          <AnimatePresence>
            {(error || successMessage) && (
              <motion.p
                ...
                exit={{ opacity: 0, y: -20 }}
                className={`absolute top-4 left-4 right-4 text-sm sm:text-base font-medium p-3 sm:p-4 rounded-lg shadow-md z-50 ${
          ...
          <div className="w-full sm:w-1/2 flex flex-col gap-4 h-full">
            ...
            <div className="flex-1 overflow-y-auto pr-2">
              {fetchingTasks ? (
                <Skeleton count={5} height={80} className="mb-3 sm:h-24" />
              ) : filteredTasks.length > 0 ? (
                filteredTasks.map((task) => (
                  <motion.div
                    key={`task-${task.id}`}
                    className="bg-white rounded-xl shadow-md p-3 sm:p-4 mb-3 cursor-pointer flex items-start"
                    whileHover={{ scale: 1.02, boxShadow: "0 6px 12px rgba(0, 128, 128, 0.15)" }}
                    whileTap={{ scale: 0.98 }}
                    onClick={() => {
                  ...
                  >
                    <div className={`w-3 h-3 rounded-sm mr-2 sm:mr-3 mt-1 ${getStatusColor(task.status || "not_started")}`} />
                    <div className="flex-1">
                      <h4 className="text-sm sm:text-base font-semibold text-gray-700 truncate">{task.title}</h4>
                      <p className="text-xs sm:text-sm text-gray-500 mt-1">Assignees: {task.assignees?.map((a) => a.name).join(", ") || "None"}</p>
          ...
          <div className="w-full sm:w-1/2 flex flex-col gap-4 h-full">
            ...
            <div className="flex gap-2">
              <motion.button
                ...
                className={`flex-1 px-3 py-1 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-base font-semibold ${
                  inputMode === "text" ? "bg-teal-600 text-white hover:bg-teal-700" : "bg-gray-200 text-gray-800 hover:bg-gray-300"
                }`}
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.95 }}
              >
              ...
              <motion.button
                ...
                className={`flex-1 px-3 py-1 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-base font-semibold ${
                  inputMode === "voice" ? "bg-teal-600 text-white hover:bg-teal-700" : "bg-gray-200 text-gray-800 hover:bg-gray-300"
                }`}
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.95 }}
              >
            ...
            <motion.button
              ...
              className={`w-full px-4 py-2 sm:px-6 sm:py-3 rounded-lg text-white text-xs sm:text-base font-semibold ${
                loading || members.length === 0 || isRecording || isTranslating
                  ...
                  <div className="flex gap-3 mt-4">
                    ...
                    <motion.button
                      ...
                      className={`flex-1 px-3 py-2 sm:px-6 sm:py-3 rounded-lg text-xs sm:text-base font-semibold ${
                        tempAssignees.length === 0 ? "bg-gray-400 cursor-not-allowed" : "bg-teal-600 text-white hover:bg-teal-700"
                      }`}
                ...
                <>
                  ...
                  <div className="flex gap-3 mt-4">
                    <motion.button
                      ...
                      className={`flex-1 px-3 py-2 sm:px-6 sm:py-3 rounded-lg text-xs sm:text-base font-semibold ${
                        loading || !editingTask.title || editingTask.assignees.length === 0
                          ? "bg-gray-400 cursor-not-allowed"
                ...
                <>
                  ...
                  <div className="flex gap-3 mt-4">
                    <motion.button
                      ...
                      className={`flex-1 px-3 py-2 sm:px-6 sm:py-3 rounded-lg text-xs sm:text-base font-semibold ${
                        loading || formData.sprints.filter((s) => s.title).length === 0
                          ? "bg-gray-400 cursor-not-allowed"
                          : "bg-teal-600 text-white hover:bg-teal-700"
                      }`}
                      whileHover={{
                        scale: loading || formData.sprints.filter((s) => s.title).length === 0 ? 1 : 1.03,
                      }}
                      whileTap={{
                        scale: loading || formData.sprints.filter((s) => s.title).length === 0 ? 1 : 0.95,
                      }}
                    >
                      Save Sprints
                    </motion.button>
                  ...
                  <div className="flex gap-3 mt-4">
                    ...
                    {!voiceInput.recording && (
                      <motion.button
                        ...
                        className={`flex-1 px-3 py-2 sm:px-6 sm:py-3 rounded-lg text-xs sm:text-base font-semibold ${
                          isTranslating || !voiceInput.title || !voiceInput.description
                            ? "bg-gray-400 cursor-not-allowed"
                            : "bg-teal-600 text-white hover:bg-teal-700"
                        }`}
                    ...
                    {!voiceInput.recording && (
                      <motion.button
                        ...
                        className={`flex-1 px-3 py-2 sm:px-6 sm:py-3 rounded-lg text-xs sm:text-base font-semibold ${
                          isTranslating || !voiceInput.title
                            ? "bg-gray-400 cursor-not-allowed"
                            : "bg-teal-600 text-white hover:bg-teal-700"
                        }`}
