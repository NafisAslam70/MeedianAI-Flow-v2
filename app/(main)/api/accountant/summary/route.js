// app/(main)/api/accountant/summary/route.js
...
export async function GET(req) {
  ...
  const suggestedOpeningCash = prev?.closingCash ?? settings?.openingCash ?? 0;

  // Prefill: receipts & expenses for that day
  ...
  return NextResponse.json({
    summary: {
      ...
      expenseBreakdown: {},
      _computed: { cashExpenses, upiExpenses, bankExpenses },
    },
  });
}
  ...
  if (existing) {
    const [upd] = await db
      ...
      .set({
        ...
        expenses,
        openingCash: oc,
        closingCash: closingCash ?? (oc + cash - (body._computedCashExpenses || 0)),
        notes,
        adjustments,
        expenseBreakdown,
    ...
    const [ins] = await db
      ...
      .values({
        ...
        expenses,
        openingCash: oc,
        closingCash: closingCash ?? (oc + cash),
        notes,
        adjustments,
        expenseBreakdown,
        isLocked: lock ? true : false,
